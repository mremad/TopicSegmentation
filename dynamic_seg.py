...
def create_content_vector(document_vec):
    for key, doc in document_vec.iteritems():
        cv_doc = []
        for sen in doc:
            cv_sen = []
...
def _get_block_centroids(document, window, dim):
    ...
    for i in xrange(0,len(document),window):
...
def segment_document_distance(document):
    ...
    #GET WINDOW CENTROIDS#
    for i in xrange(0,len(document),3):
        j = i - (window/2)
...
def main_segment_window(args, doc_vec_repr, bnds):
    ...
    for key, doc in doc_vec_repr.iteritems():
        ...
        print "**********"
        print '\n'.join(['%i' % (hyp_bnds[n]) for n in xrange(len(hyp_bnds))])
        print "----------"
