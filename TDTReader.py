...
class TDTCorpus:
    ...
    def construct_char_bnds(self):
        for key, text in self.text_corpus_bnds.iteritems():
            text = ' '.join(text)
...
def main_process(load_from_disk):
    ...
    if not load_from_disk:
        ...
        corpus.read_tkn_sample_files('/home/mohamed/Desktop/tdt2_em_v4_0/tdt2_em/tkn')
        ...
        corpus.read_bnd_sample_files('/home/mohamed/Desktop/tdt2_em_v4_0/tdt2_em/tkn_bnd')
    ...
    for key,val in corpus.text_corpus_bnds.iteritems():
        key = key.strip()
...
def evaluate_seg(corpus, out_bnds, gap_scores=[], smooth_scores=[], depth_scores=[]):
    ...
    for key, val in  out_bnds.iteritems():
